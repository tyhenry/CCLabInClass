...
void ofApp::setup(){
	...
	font.loadFont("Sudbury_Basin_3D.ttf", 32, true, false, true);
...
void ofApp::update(){
	...
	if (finder.blobs.size() > 0) {
		...
		worldHeadX = ofMap(cameraHeadX, 0, video.getWidth(), windowBottomRight.x, windowBottomLeft.x);
		
		//low y in camera is +ve y in world
		worldHeadY = ofMap(cameraHeadY, 0, video.getHeight(), windowTopLeft.y, windowBottomLeft.y);
    ...
    beatPan = ofMap(worldHeadX, -0.097f, 0.1f, -1.0f, 1.0f, true);
    beats.setPan(beatPan);
    beatSpeed = ofMap(worldHeadY, -0.057f, 0.059f, 0.5f, 1.5f, true);
...
void ofApp::drawScene(bool isPreview){
	...
	for (float z = 0.0f; z > -40.0f; z-= 0.1f){
        ...
        ofDrawBitmapString("BEATS!!", 0, 0, z);
	}
	ofPopStyle();
...
void ofApp::draw(){
    ...
    font.drawString("beats !!", ofGetWidth()-300,ofGetHeight()-150);
    string tempStr = "pct done: "+ofToString(beats.getPosition()) +"\n\nWorldHeadX: "+ofToString(worldHeadX,4) +"\nWorldHeadY: "+ofToString(worldHeadY,4) +"\n\nclick and drag mouse to change:\nbeatPan: "+ofToString(beats.getPan()) +"\nbeatSpeed: "+ofToString(beats.getSpeed());
	ofDrawBitmapString(tempStr, ofGetWidth()-300,ofGetHeight()-120);
...
void ofApp::mouseDragged(int x, int y, int button){
    ...
    beatPan = ofMap(x, 0.0f, ofGetWidth(), -1.0f, 1.0f, true);
    beats.setPan(beatPan);
    beatSpeed = ofMap(y, 0.0f, ofGetHeight(), 1.5f, 0.5f, true);
    beats.setSpeed(beatSpeed);
}
